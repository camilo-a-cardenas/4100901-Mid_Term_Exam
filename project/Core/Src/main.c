...
int I_presion=0;
int D_presion=0;
uint8_t DATAI[] = "Izquierda, UART!\r\n";
uint8_t DATAD[] = "Derecha, UART!\r\n";
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  /* Prevent unused argument(s) compilation warning */
	 if(GPIO_Pin==BOTI_Pin){
		  I_presion++;
	  }
	  if(GPIO_Pin==BOTD_Pin){
	  	  D_presion++;
	    }
	  ...
	  if(I_presion!= 0){
		  if(I_presion>=1){
			  for(int i=1;i<=0;i++){
				  ...
				  HAL_UART_Transmit(&huart2, DATAI, sizeof(DATAI)- 1, HAL_MAX_DELAY);

				  if (D_presion!=0){
					  HAL_GPIO_WritePin(LEDI_GPIO_Port, LEDI_Pin, 1);
					  I_presion=0;
					  break;
				  if (I_presion >= 2 &&  i<=2 && D_presion == 0 ){
					  while(1){
					  		HAL_GPIO_TogglePin(LEDI_GPIO_Port, LEDI_Pin);
					  		HAL_Delay(250);
					  		HAL_UART_Transmit(&huart2, DATAI, sizeof(DATAI)- 1, HAL_MAX_DELAY);
	  ...
	  if(D_presion!= 0){
	  		  if(D_presion>=1){
	  			  for(int i=1;i<=0;i++){
	  				  ...
	  				  HAL_Delay(250);
	  				  HAL_UART_Transmit(&huart2, DATAD, sizeof(DATAD)- 1, HAL_MAX_DELAY);
	  				  if (I_presion!=0){
	  					  HAL_GPIO_WritePin(LEDI_GPIO_Port, LEDI_Pin, 1);
	  					  D_presion=0;
	  					  break;
	  				  if (D_presion >= 2 &&  i<=2 && I_presion == 0 ){
	  					  while(1){
	  					  		HAL_GPIO_TogglePin(LEDD_GPIO_Port, LEDD_Pin);
	  					  		HAL_Delay(250);
	  					  	    HAL_UART_Transmit(&huart2, DATAD, sizeof(DATAD)- 1, HAL_MAX_DELAY);
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pins : BOTI_Pin BOTD_Pin */
  GPIO_InitStruct.Pin = BOTI_Pin|BOTD_Pin;
